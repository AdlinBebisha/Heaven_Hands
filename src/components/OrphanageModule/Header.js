...
const Header = ({ title, showBackButton, onBack, user: initialUser }) => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const response = await fetch(`http://localhost:8080/api/orphan-profile/${user.email}`);
        if (response.ok) {
  ...
  return (
    <header className="p-4 bg-[#990011] text-white flex items-center justify-between flex-wrap">
      {/* Back Button */}
      {showBackButton && (
        <button
      ...
      <nav className="flex items-center mt-2 sm:mt-0">
        ...
        <Link
          className="mx-2 hover:underline text-sm sm:text-base"
          to="/orphan-notification"
          aria-label="View Notifications"
        >
          Notifications
        </Link>
        <Link to="/orphan-profile" className="ml-3" aria-label="View Profile">
          <img
